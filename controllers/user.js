...
const resetPassword = async(req,res) =>{
  ...
  if(!forgetPassToken){
    res.status(400).send("<html> <body><h1> Verify your email first! </h1></body></html>")
  }
  ...
  if(!userToken){
    ...
    res.status(400).send({message : 'user not found'});
      ...
      const userUpdate = await User.updateOne({_id : userid._id}, {

        $set : {password : hashedPassword}
      });

      if(userUpdate.matchedCount > 0)
      res.status(200).send("<html> <body><h1> Password Updated !</h1></body></html>");
    }
...
const forgetPassword = async(req,res) =>{
  ...
  if(!userEmail){
    res.status(400).send("<html> <body><h1>User does not exist!</h1></body></html>");
  }
  else{
    ...
    if(!userToken){
     ...
     const path = __dirname + '\\html pages\\forgetPassword.html'; 
      ...
      res.status(200).send("<html> <body><h1> Reset Password link send to your email </h1></body></html>");
...
const verifyUser = async(req,res) =>{
  ...
  if(!userToken){
    ...
    await User.updateOne({_id : userToken.userID}, {

      $set : {isVerified : true}
    });

    res.status(200).send("<html> <body><h1> Successfully Verified! <p> Go Back to the login page </p></h1></body></html>");
  }
...
const signup = async (req, res, next) => {
  ...
  if(user){
    ...
    res.status(200).send("<html> <body><h1> User has been successfully registered!</h1></body></html>");
